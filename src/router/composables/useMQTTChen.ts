...
class GlobalMQTTClient {
  ...
  private subs = new Map<string, { qos: QoS; refs: number }>([['moubing', { qos: 2, refs: 1 }]])
  ...
  private connect(): void {
    ...
    this.client.on('connect', () => {
      ...
      console.log(this.client?.connected, 'connect status')
      this.emitStatus('connected')
      this.subs.forEach((entry, topic) => {
        this.client!.subscribe(topic, { qos: entry.qos }, (err) => {
          if (!err) {
            console.log(`订阅${topic}主题成功`)
          } else {
            console.log('订阅出错' + err.message)
          }
        })
      })
    })
